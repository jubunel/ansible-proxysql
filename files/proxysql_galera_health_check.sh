...
#
NUM_WRITERS=$(status_query "select count(*) FROM runtime_mysql_servers WHERE status='ONLINE' AND hostgroup_id=$1;")

#
# Check how many readers are online
#
NUM_READERS=$(status_query "select count(*) FROM runtime_mysql_servers WHERE status='ONLINE' AND hostgroup_id=$2;")

if [ -z "$NUM_WRITERS" ] || [ "$NUM_WRITERS" = "0" ]; then
     http_no_access "No writers online"
elif [ -z "$NUM_READERS" ] || [ "$NUM_READERS" = "0" ]; then
     http_no_access "No readers online"
else
     http_ok "We have readers and writers"
fi
